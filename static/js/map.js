...
if (locations.length > 0) {
  ...
  map = L.map("map").setView([firstLocation.lat, firstLocation.long], 13);
} else {
...
if (locations.length > 0) {
  var firstLocation = locations[0];
  L.marker([firstLocation.lat, firstLocation.long])
    .addTo(map)
    .bindPopup("Latitude: " + firstLocation.lat + "<br>Longitude: " + firstLocation.long)
    .openPopup();
}
...
if (locations.length > 1) {
  ...
  L.marker([lastLocation.lat, lastLocation.long])
    .addTo(map)
    .bindPopup("Latitude: " + lastLocation.lat + "<br>Longitude: " + lastLocation.long)
    .openPopup();
}
